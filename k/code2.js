  if(!t){
    ...
    spawnFreq = 200
    spawnSection = () => {
      for(let i=br*rw*cl;i--;){
        X = ((i%cl)-cl/2 + .5)*sp
        Y = (((i/cl|0)%rw)-rw/2 + .5)*sp
        Z = 40*sp
        if(Math.hypot(X,Y)<Math.hypot(rw**2/10)*2)sections = [...sections, [X,Y,Z,1]]
      }
    }
    ...
    iCannonsc = 3
    iCannonsr = 100
    base_cannon = Cylinder (1,6,10,10)
    cannons = Array(iCannonsc).fill().map((v,i) => {
      tx = S(p=Math.PI*2/iCannonsc*i)*iCannonsr
      ty = C(p)*iCannonsr
      tz = 0
      shp = JSON.parse(JSON.stringify(base_cannon)).map(v=>{
        pt = 0
        rl = Math.PI*2/iCannonsc*i
        yw = 0
        v.map(q=>{
          X = q[0]
          Y = q[1]
      ...
      return [tx,ty,tz,shp,rl,pt,yw]
    })

    P = Array(iCannonsc).fill().map(v=>[])
    pfreq = 0, iPv = 4
    spawnP = () => {
      cannons.map((v, i)=>{
        tx = v[0]
  ...
  oX=0, oY=0, oZ=Math.min(300,Math.max(150,(.3+C(t/2))*1000))
  Rl=C(t/3)*2, Pt=-t/8, Yw=S(t/4)*2+Math.PI/4
  x.fillStyle='#0008'
  x.fillRect(0,0,c.width,c.height)
  x.lineJoin = x.lineCap = 'round'
  
  if(!((t*60|0)%pfreq)) spawnP()
  if(!((t*60|0)%spawnFreq))spawnSection()
  ...
  cannons.map(v=>{
    ...
    Y = v[1]
    Z = v[2]
    v[4]+=t1=Math.min(.1,Math.max(-.1,-C(t/4)))+.09
    R(t1,0,0)
    tx = v[0] = X
    ty = v[1] = Y
    tz = v[2] = Z
    v[3].map(q=>{
      ...
      stroke('#0f8','#0f82',15,true)
    })
  })

  sections = sections.filter(v=>v[2]>-200 && v[3]>0)
  ...
  sections.map(v=>{
    ...
    base_shape.map(q=>{
      ...
      stroke('', `hsla(${260},99%,50%,${1/(1+Math.abs(Z_)**6/9999999999999)}`, 4, false)
    })
  })
  x.globalAlpha = 1

  P = P.map(P=>{
    return P.filter(v=>v[6]>.05)
  })
    ...
    P.map((v,i)=>{
      ...
      sections.map(q=>{
        if(q[3]){
          ...
          Y2 = q[1]
          Z2 = q[2]
          d_=Math.hypot(a=X2-X1,b=Y2-Y1,e=Z2-Z1)
          d = (1+d_)**4.25
          v[3] += a*2e4/d
          v[4] += b*2e4/d
          v[5] += e*2e4/d
      ...
      if(Z>0){
        l = Q()
        if(i && Math.hypot(X1-olx,Y1-oly,Z1-olz)<iPv*16){
          x.beginPath()
          x.lineTo(...l)
          X = olx
          Y = oly
          ...
          R(Rl,Pt,Yw,1)
          if(Z>0) x.lineTo(...Q())
          stroke('#0f85','',15000/Z*v[6]**.5,true)
        }
        s = Math.min(1e3, 2e4/Z*(.1+v[6]**2))
        x.fillStyle = '#4400ff03'
        x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
        s/=3
        x.fillStyle = '#00ff8820'
        x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
        s/=3
        x.fillStyle = '#88ffffaa'
        x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
        olx=X1
        oly=Y1
        olz=Z1
  ...
  sparks.map(v=>{
    ...
    if(Z>0){
      l = Q()
      s = Math.min(1e3, 4e4/Z*v[6])
      x.fillStyle = '#FF000001'
      x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
      s/=4
      x.fillStyle = '#FFff0008'
      x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
      s/=4
