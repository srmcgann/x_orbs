    ...
    <script>
      ...
      async function Draw(){
        ...
        if(firstRun){
          ...
          burst = new Image()
          burst.src = "proxy.php?url=https://srmcgann.github.io/temp/burst.png"
          ...
          tgtcol = location.href.toUpperCase().split('TGTCOL=')[1]
          ...
          resAdjust=window.location.href.toUpperCase().split('RES=')[1]
          if(typeof resAdjust !== 'undefined' && resAdjust.length){
            resAdjust = resAdjust.split('&')[0]
          } else {
            resAdjust = 1
          }
          res = .06 * resAdjust
          ...
          src.onload = () => {
            loaded = true
            console.log('loaded')
            buffer.width = (twd=Math.min(1400, src.width)) * res | 0
            buffer.height = src.height * (twd / src.width) * res | 0
            bctx.drawImage(src, 0, 0, buffer.width, buffer.height)
            data = bctx.getImageData(0, 0, buffer.width, buffer.height)
          }
          bctx = buffer.getContext('2d')
          if(location.href.indexOf('src=')!==-1){
            tsrc = location.href.split('src=')[1].split('&')[0]
          }else{
            tsrc = 'https://srmcgann.github.io/drawings/IMG_20231002_132945.jpg'
          }
          src.src = 'proxy.php?url=' + tsrc
          ...
          plot = l => {
            l2 = l*4
            ls = sphereizeRad - (mapHeight - (data.data[l2+0]+data.data[l2+1]+data.data[l2+2]) / 3 / 255 * mapHeight)**.2
            X = ((l%w)-w/2+.5) * sp
            Y = ((l/w|0)-h/2+.5) * sp
            Z = -sphereizeRad
            d = Math.hypot(sphereX-X, sphereY-Y, sphereZ-Z)
            X = (sphereX + X) / d * ls
            Y = (sphereY + Y) / d * ls
            Z = sphereizeRad/2+(sphereZ + Z) / d * ls
            R(Rl,Pt,Yw,1)
            a = [...a, [X,Y,Z]]
            //R(Rl,Pt,Yw,1)
            //if(Z>0) x.lineTo(...Q())
        ...
        oX=0, oY=0, oZ=Math.min(24, Math.max(17,(.3+C(t))*100))
        Rl=0, Pt=C(t)/1.25, Yw=-S(t*2.5)
        ...
        if(loaded){
          ...
          for(i=0; i<data.data.length; i+=4){
            ...
            if(j%w && j<w*h-w){
              ...
              diff=(Math.abs(col[0]-tgtRGBA[0])+Math.abs(col[1]-tgtRGBA[1])+Math.abs(col[2]-tgtRGBA[2]))/3
              col[3] = diff>tol?.95:0
              if(diff>tol){ //lum>25){
                tz = 0
                a = []
                ...
                B = [...B, [a, col, [ax/4, ay/4, az/4]]]
              }
            }
          }

          B.sort((a,b)=> b[2][2] - a[2][2])
          ...
          B.map(v=>{
            ...
            stroke('',`rgba(${v[1][0]},${v[1][1]},${v[1][2]},${v[1][3]})`)
          })

          x.drawImage(src,10,10,src.width/6,src.height/6)
          t+=1/60
        }
