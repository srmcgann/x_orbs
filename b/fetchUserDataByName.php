<?php 
  ...
  if($name){
    ...
    if($includePrivate){
      ...
      $sql="SELECT id FROM games WHERE userID = " . $row['id'];
      $res = mysqli_query($link, $sql);
      $totalRecords = mysqli_num_rows($res);
      $totalPages = floor((intval($totalRecords-1) / $maxResultsPerPage)) + 1;

      $sql1=$sql = "SELECT * FROM games WHERE userID = " . $row['id'] . ' ORDER BY date DESC LIMIT ' . $start . ', ' . $maxResultsPerPage;
    } else {

      $sql="SELECT id FROM games WHERE private = 0 AND userID = " . $row['id'];
      $res = mysqli_query($link, $sql);
      $totalRecords = mysqli_num_rows($res);
      $morePages = false;
      $totalPages = floor((intval($totalRecords-1) / $maxResultsPerPage)) + 1;
      ...
      $sql = "SELECT * FROM games WHERE userID = " . $row['id'] . ' AND private = 0 ORDER BY id DESC LIMIT ' . $start . ', ' . $maxResultsPerPage;
    }
    $res = mysqli_query($link, $sql);
    $posts = [];
