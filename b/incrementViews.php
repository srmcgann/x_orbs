<?php 
  ...
  $sql = "DELETE FROM gamesViews WHERE time < " . $t_minus;
  ...
  if($_SERVER['REMOTE_ADDR']){
    ...
    $sql='SELECT time FROM gamesViews WHERE decIP = ' . $decIP . ' AND postID = ' . $postID . ' ORDER BY time DESC';
    $sql1=$sql;
    ...
    if($go){
      $row = mysqli_fetch_assoc($res);
      $sql = 'UPDATE games SET views = views + 1 WHERE id = ' . $postID;
      mysqli_query($link, $sql);
      $time = time();
      $sql = 'INSERT INTO gamesViews (decIP, postID, time) VALUES('.$decIP.','.$postID.','.$time.');';
      mysqli_query($link, $sql);
    }
  }
  echo json_encode([$go]);
