<?php 
  ...
  if(sizeof($tokens)){
    ...
    $confirmed = false;
    if($loggedinUserName){
      $sql_ = $sql = 'SELECT * FROM users WHERE name LIKE "' . $loggedinUserName . '" AND passhash = "' .  $passhash . '"';
  ...
  $sql1 = $sql .= ' ORDER BY id DESC LIMIT ' . $start . ', ' . $maxResultsPerPage;
  $res = mysqli_query($link, $sql);
  ...
  echo json_encode([$posts, $totalPages, $page, $totalRecords, $sql1, $loggedinUserData]);
?>
